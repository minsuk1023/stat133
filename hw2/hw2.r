...
text(wr_1944, wr1500m$times_sec[wr1500m$new_year == wr_1944], labels = wr1500m$athlete[wr1500m$new_year == wr_1944], adj = 1, cex = 0.7, col = "green")
text(wr_1998, wr1500m$times_sec[wr1500m$new_year == wr_1998], labels = wr1500m$athlete[wr1500m$new_year == wr_1998], adj = 1, cex = 0.7, col = "blue")
...
plot(wr1500m$new_year, wr1500m$times_sec, type = "s", xlim = c(1892,2015), xlab = "Year", ylab = "Record Times (sec)", main = "World Record Times in Men's 1500 meter")

abline(v = wr_1944, col = "grey")
abline(v = wr_1998, col = "grey")
points(x = c(wr_1998, 2012+9/12), y = rep(wr1500m$times_sec[wr1500m$new_year == wr_1998], 2), type = "l")
text(wr_1944-0.5, wr1500m$times_sec[wr1500m$new_year == wr_1944], labels = wr1500m$athlete[wr1500m$new_year == wr_1944], adj = 1, offset = 2, cex = 0.7, col = "green")
text(wr_1998-0.5, wr1500m$times_sec[wr1500m$new_year == wr_1998], labels = wr1500m$athlete[wr1500m$new_year == wr_1998], adj = 1, offset = 2, cex = 0.7, col = "blue")
...
with(SO2012Ctry[SO2012Ctry$Total > 0, ], symbols(log(pop), log(GDP_per_person), circles = sqrt(Total)/40, inches = FALSE))
...
with(SO2012Ctry[SO2012Ctry$Total > 0, ], symbols(log(pop), log(GDP_per_person), circles= sqrt(Total)/40, inches = FALSE, xlab = "log of population", ylab = "log of GDP per person", main = "Population vs GDP per person"))
with(SO2012Ctry[SO2012Ctry$Total == 0, ], points(log(pop), log(GDP_per_person), pch = "."))
text(log(SO2012Ctry$pop)[top5], log(SO2012Ctry$GDP_per_person)[top5], labels = SO2012Ctry$ISO[top5], cex = seq(.8, .4, by = -.1), col = "blue")
...
world <- map(database = "world", fill = TRUE, col = "light grey")
symbols(wonMedal$longitude, wonMedal$latitude, circles = sqrt(wonMedal$Total)/1.5, add = TRUE, inches = FALSE)
...
world <- map(database = "world", fill = TRUE, col = "light grey")
symbols(wonMedal$longitude, wonMedal$latitude, circles = sqrt(wonMedal$Total)/1.5, add = TRUE, inches = FALSE, bg = myGold)
...
### Sport : nominal
### Country : nominal
### MoreThan1Sport : ordinal or nominal 
...
barplot(table(athletes$Sex, athletes$Sport)[, orderSport], beside = TRUE, cex.names = 0.8, las = 3, ylim = c(0,1200), main = "Olympic Sports by Gender")

...
name <- "Minsuk Sung"
github.name <- "minsuk1023"
email.address <- "minsuk1023@berkeley.edu"
...
i=1
while(i<=10){
  inlist = seq(from = i, to=1000, by =i)
  y[[i]] = inlist
  i=i+1
}
...
# i.e. each element is one of the numbers 1 through 6, with equal probability.
  set.seed(31415)
v <- sample(seq(from=1,to=6, by=1), size=100, replace = TRUE, prob= rep(1/6, times=6))
...
set.seed(2718)
w <- c("aa", "ab", "ac","ad", "ae", "af","ag", "ah", "ai","aj", "ak", "al","am", "an", "ao","ap", "aq", "ar","as", "at", "au","av", "aw", "ax","ay", "az")
...
# Diet 1 at time 0
cw1 <- ChickWeight$weight[ChickWeight$Diet==1]
  
# [1 pt]
# Create [cw2] a subset of ChickWeight with only Chicks that
# weighed more than 41 at Time 0
cw2 <- ChickWeight$weight[ChickWeight$Time == 0 & ChickWeight$weight > 41]
...
cw4 <- sample(ChickWeight$weight, size = 200, replace = FALSE)
...
colsmtcars <- rainbow(length(unique(mtcars$am)))
plot(x = mtcars$wt, y = mtcars$disp, xlab= "weight of the car in lb/1000", ylab = "engine displacement in cu. in.", col = colsmtcars)
...
plot(WorldBank$fertility.rate, WorldBank$life.expectancy, pch=".", col=colsWorldBank, xlab= "fertility rate", ylab = "life expectancy")
abline(v=2.1)
...
plot(WorldBank$fertility.rate[WorldBank$year == "2013"],
     WorldBank$life.expectancy[WorldBank$year == "2013"],
     pch=".", col=colsWorldBank, xlab= "fertility rate", ylab = "life expectancy")
...
hist(WorldBank$GDP.per.capita.Current.USD[WorldBank$lending =="IDA"],xlab="GDP for IDA",prob =T, main = "IDA Histogram")
...
maxAbsDiffTwoconsecutiveDays = function(rainlist) {
  i = 2
  maxDiff = 0
...
noRain = function(rainlist) {
  return(length(rainlist[rainlist > 0]))
}

prop.rain <- unlist(lapply(rain, noRain))
...
GenNorm <-function(mean = 0, sd = 1, n = 1000, plot.his = TRUE) {
  toReturn = rnorm(n = n, mean =mean, sd = sd)
  if (plot.hist) {
    plot(type = "h", x =toReturn)
  }
  return(toRetrun)
}
...
#    mean(cases)-mean(controls) for one permutation
 ...
 PermDiff <- function(cases, controls, k=5000){
   ...
   i =1
   while (i <=k) {
   randomsampling = sample(c(cases,controls), size = length(cases) + length(controls))
   randomx = randomsampling[1:length(cases)]
   remain= randomsapmling[k+1: length(randomsapmling)]
   
   toreturn[i] = mean(randomx) - mean(remain)
  i = i +1
   }
  return(toreturn)
...
NumJackpot <- function(k, B){
  numberofwinners = vector(length = 8)
  for (i in 1:8) {
    winningnumber = sample(1:19, size = 3)
    winners = 0
    for (players in 1:k) {
      playernumber = sample(1:19, size =3)
      if(unique(sort(winningnumber) == sort(playernumber)) == 1) {
        winners = winners + 1
      }
    }
    numberofwinners[i] = winners
  }
...
# Create a vector [match1] with the index for all matches to "A*T" or "G*T"
 match1 <- c("AGGATGATT",   "AGTTTCGTA",  "CTAAGTGAC", "GTGGGACC",  "TAGATTACC")

# [2 pts]
# Create a vector [dna2] where you have removed all entries whose length is not a multiple of 3
 dna2 <-c("AGGATGATT", "AGCCTTAGC", "AGTTTCGTA", "CTAAGTGAC", "GGTAGAGAC", "TAGATTACC")
